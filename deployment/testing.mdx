---
title: "Testing Spades Online: Unit, Integration, and WebSocket Tests"
...
<Note>
  **Unit tests always run.** Integration tests are automatically skipped when `DATABASE_URL` is not set in the environment. Integration tests that also require Redis are skipped when `REDIS_URL` is not set. You can run the full unit suite locally without any external services.
</Note>
