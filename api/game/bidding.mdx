---
title: "Bidding Phase API: Bid, Reveal Hand, and Blind Nil Exchange"
sidebarTitle: "Bidding"
description: "Place bids (0–13, Nil, or Blind Nil), reveal your hand before bidding, and execute the two-step Blind Nil card exchange via the Spades Online API."
---
...
Place a bid during the bidding phase. Valid bid values are `0` (Nil), `-1` (Blind Nil — only if eligible), or an integer from `1` to `13`.

**Required headers:** `x-session-id`, `x-player-id`

### Body

<ParamField body="bid" type="integer" required>
  The bid to place.
  - `0` — Nil (player commits to taking zero tricks; worth ±50 points)
  - `-1` — Blind Nil (player bids before seeing their hand; worth ±100 points; only allowed when your team is at least 100 points behind)
  - `1`–`13` — Regular bid for that number of tricks
</ParamField>
...
Reveals the player's hand so they can inspect their cards before deciding whether to bid Blind Nil. This endpoint is only meaningful for players who are eligible for Blind Nil (their team is at least 100 points behind). Calling `reveal-hand` **forfeits Blind Nil eligibility** — after revealing, the player must place a regular bid (0–13) instead.
...
<ParamField body="cards" type="array" required>
  An array of exactly **2 card strings** to transfer. Cards must exist in the submitting player's current hand. Card format: rank + suit initial (e.g. `"AS"` for Ace of Spades, `"TD"` for 10 of Diamonds). See [Hand representation](/api/game/state#hand-representation) for the full encoding reference.
</ParamField>
...
<Steps>
  ...
  <Step title="Blind Nil check">
    Before bidding, any player whose team is at least 100 points behind receives `blindNilEligible: true` in their state view and does **not** receive `myHand`. They must either call `POST /reveal-hand` (to see their hand and forfeit Blind Nil) or bid `-1` (Blind Nil) without looking.
  </Step>
  ...
  <Step title="Blind Nil exchange (if applicable)">
    If any player bid Blind Nil (`-1`), the game moves to `phase: "blind_nil_exchange"`. The Blind Nil player calls `POST /blind-nil-exchange` with 2 cards to send to their partner (Step 1). Their partner then calls the same endpoint with 2 cards to return (Step 2). If the partner is a bot, Step 2 is auto-processed. If both partners bid Blind Nil, each pair completes their exchange sequentially.
  </Step>
