---
title: "Join, Sit, and Invite Players to a Spades Table"
sidebarTitle: "Join Table"
description: "How to sit at a table, use join links and spectator links, send in-app invites, and arrive as an observer. Covers POST /api/tables/:tableId/sit, join-link, and spectator-link endpoints."
---
...
Arrive at a table as an observer without a link. The table must have spectating enabled. If the player is already seated or already an observer, the call is idempotent and succeeds.
...
**Path parameter:** `token` — join link token from `GET /api/tables/:tableId/join-link` or an in-app invite.
...
Validates a spectator link token and adds the authenticated player to the table as an observer. The player is permanently marked as spectator-only for this table session — they cannot sit down even if a seat is free.
...
Invitee-only. Declines a pending invite. All associated Redis keys (the invite record, the duplicate-check key, the join-link token, and the invited-list entry) are deleted. A `INVITE_DECLINED` event is published to the host's notification channel so their UI can update.
